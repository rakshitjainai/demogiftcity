...
export default function SectionDetail() {
  ...
  const handleSelectProvision = (chapId, provNumber) => {
    ...
    if (provNumber) {
      ...
      navigate(`/understand/${selectedSlug}/${chapId || activeChapterId}/${provNumber}`, { replace: true });
    }
    ...
    <div className="min-h-screen bg-paper flex flex-col font-sans text-ink">
      ...
      <div className="bg-white border-b border-line px-4 sm:px-6 py-2.5 flex items-center justify-between text-xs text-ink-soft">
        <div className="flex items-center gap-2 overflow-x-auto no-scrollbar py-0.5">
          ...
          <span>/</span>
          <Link to="/understand" className="hover:text-forest font-medium">RegLens</Link>
          <span>/</span>
          <span className="text-forest font-semibold truncate max-w-[200px] sm:max-w-md">
            {currentReg?.short_title || currentReg?.title}
          </span>
          <span>/</span>
          <span className="text-ink font-bold">Reg {activeProvision?.number || '1'}</span>
        </div>

        <Link
          to="/understand"
          className="text-forest font-semibold hover:underline hidden sm:inline"
        >
        ...
        <main className="col-span-1 lg:col-span-6 px-4 sm:px-6 lg:px-0 space-y-6">
          ...
          {loading ? (
            <div className="p-12 text-center text-ink-soft bg-white rounded-2xl border border-line">
              ...
              <p className="text-sm font-medium">Loading provision...</p>
            </div>
          ) : (
            <>
